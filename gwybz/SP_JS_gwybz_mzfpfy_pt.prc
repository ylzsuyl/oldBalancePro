CREATE OR REPLACE PROCEDURE SP_JS_gwybz_mzfpfy_pt
(v_qsrq00 IN char,
v_jzrq00 IN char,
v_ret OUT number)
as
...
begin
open mycursor for
Select * From yy_mzsfb0 Where gwyjb0 in ('11','12') And sfrq00>=v_qsrq00 And sfrq00<=v_jzrq00 
and JSFSBH='01' and sfrq00>='20100201' and gwybz0 is not null and gwybz0<>0 and BQBM00 is null        
and qsrxm0 is not null and qsrq00 is not null Order By sfrq00;
    loop
        ...
        commit;
        --取公务员补助比例
        select mlnbl0,mlwbl0 into ls_mlnbl0,ls_mlwbl0  from bm_gwyblb where jsfsbh='01' and gwyjb0=v_mine.gwyjb0 and dqbh00=v_mine.dqbh00
        AND v_mine.Sfrq00 BETWEEN QSRQ00 AND JZRQ00;
        --计算实际公务员补助费用
         ls_CZBZN0:=(ls_ybfy00+ls_ybgrfy-v_mine.jjzfe0-v_mine.DBJJZF)*ls_mlnbl0;
        --ls_CZBZW0:=ls_fybfy0*ls_mlwbl0;
         ls_CZBZW0:=v_mine.gwybz0-ls_CZBZN0;
         ls_GWYBZ0:=ls_CZBZN0+ls_CZBZW0;
         ls_GWBZN0:=0;
         ls_GWBZW0:=0;
        --插入公务员补助表
        SELECT COUNT(1) INTO LS_CNT FROM yy_mzfpfy_gwyb0_pt_other WHERE DJLSH0 = v_mine.djlsh0;
        
        IF LS_CNT = 0 THEN
        
        insert into yy_mzfpfy_gwyb0_pt_other (djlsh0,ybfy00,ybgrfy,fybfy0,ID0000,XMING0,BCBXF0,JJZFE0,ZHZFE0,
