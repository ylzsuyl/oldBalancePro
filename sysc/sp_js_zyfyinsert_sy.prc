create or replace procedure sp_js_zyfyinsert_sy
(
  pi_fwwdbh  in char,
  pi_czy000  in char,
  po_ret out number
...
begin
  ...
  begin  --清空临时表数据
    delete from js_temp_zyfytj_sy where fwwdbh like pi_fwwdbh||'%';
    delete from JS_TEMP_ZYTS_sy where fwwdbh like pi_fwwdbh||'%';
    UPDATE yy_zysfb0
      set   qsrq00 = null
