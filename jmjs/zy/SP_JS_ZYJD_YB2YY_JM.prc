CREATE OR REPLACE PROCEDURE SP_JS_ZYJD_YB2YY_JM
(
...
BEGIN
	...
	SELECT COUNT(*) INTO LN_COUNT FROM VW_JS_ZYJS00_JM
   ...
   --IF LN_WDLB00='04' THEN  --零售药店没有住院
   --   v_sqlerrm :='网点为药店，没有住院结算';
    --  RAISE ERR_YD_ZYJS;
   --END IF;
   -----------根据不同分中心分别计算--------------------------
   OPEN C_1;
...
EXCEPTION   WHEN ERR_INS_ZYJD THEN
               PO_SIGN := -7421;
    	         ...
    	         INSERT INTO JS_ERRLOG(CWH000,JDCWXX,XXCWXX,CCDF00,CZDX00,CZY000) VALUES
         		   (PO_SIGN,'插入结算门诊季度表时出错!',V_SQLERRM,'结算->SP_JS_ZYJD_YB2YY_JM',pi_FWWDBH,pi_czyuan);
       	      COMMIT;
       	   WHEN ERR_SEL_HBCS THEN
               PO_SIGN := -7422;
    	         ROLLBACK;
    	         INSERT INTO JS_ERRLOG(CWH000,JDCWXX,XXCWXX,CCDF00,CZDX00,CZY000) VALUES
         		   (PO_SIGN,'从表BM_JSBLB0_JM查找支付能力系数时错误!',V_SQLERRM,'月度结算->SP_JS_ZYJD_YB2YY_JM',pi_FWWDBH,pi_czyuan);
       	      COMMIT;
       	   WHEN ERR_JS_ZRYFY THEN
               PO_SIGN := -7423;
    	         ROLLBACK;
    	         INSERT INTO JS_ERRLOG(CWH000,JDCWXX,XXCWXX,CCDF00,CZDX00,CZY000) VALUES
         		   (PO_SIGN, '计算转入院费用时错误!',V_SQLERRM,'月度结算->SP_JS_ZYJD_YB2YY_JM',pi_FWWDBH,pi_czyuan);
       	      COMMIT;
            ...
            WHEN ERR_YD_ZYJS THEN
               ...
               ROLLBACK;
    	         INSERT INTO JS_ERRLOG(CWH000,JDCWXX,XXCWXX,CCDF00,CZDX00,CZY000) VALUES
         		   (PO_SIGN,'网点为药店，没有住院结算!',V_SQLERRM,'住院月度结算->SP_JS_ZYJD_YB2YY_JM',pi_FWWDBH,pi_czyuan);
       	      COMMIT;
            WHEN OTHERS THEN
    	         ROLLBACK;
               PO_SIGN := -7423;--门诊季度结算出错不详
               V_SQLERRM := SQLERRM;
    	         INSERT INTO JS_ERRLOG(CWH000,JDCWXX,XXCWXX,CCDF00,CZDX00,CZY000) VALUES
      		      (PO_SIGN,'门诊季度结算发生以外错误!',V_SQLERRM,'结算->SP_JS_ZYJD_YB2YY_JM',pi_FWWDBH,pi_czyuan);
       	      COMMIT;
END SP_JS_ZYJD_YB2YY_JM;
/
