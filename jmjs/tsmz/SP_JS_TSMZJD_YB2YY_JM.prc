CREATE OR REPLACE PROCEDURE SP_JS_TSMZJD_YB2YY_JM
(
            ...
            SELECT count(*),nvl(SUM(TSMZCS),0),nvl(SUM(fybfy0),0),nvl(SUM(fybzh0),0),
             ...
             if ln_count>0 then
               ...
               Begin
         	     ...
         	     INSERT INTO JS_TSMZJD_YB2YY_JM
            	   (
...
EXCEPTION   WHEN ERR_INS_MZJD THEN
               PO_SIGN := -7411;
    	         ...
    	         INSERT INTO JS_ERRLOG(CWH000,JDCWXX,XXCWXX,CCDF00,CZDX00,CZY000) VALUES
         		   (PO_SIGN,'插入结算特殊门诊月度表时出错!',V_SQLERRM,'结算->SP_JS_TSMZJD_YB2YY',pi_FWWDBH,pi_czyuan);
       	      COMMIT;
       	   WHEN ERR_SEL_HBCS THEN
               PO_SIGN := -7412;
    	         ROLLBACK;
    	         INSERT INTO JS_ERRLOG(CWH000,JDCWXX,XXCWXX,CCDF00,CZDX00,CZY000) VALUES
         		   (PO_SIGN,'从表BM_JSBLB0_JM查找支付能力系数时错误!',V_SQLERRM,'结算->SP_JS_TSMZJD_YB2YY',pi_FWWDBH,pi_czyuan);
       	      COMMIT;
            ...
            WHEN OTHERS THEN
               ...
               V_SQLERRM := SQLERRM;
    	         INSERT INTO JS_ERRLOG(CWH000,JDCWXX,XXCWXX,CCDF00,CZDX00,CZY000) VALUES
      		      (PO_SIGN,'特殊门诊结算发生以下错误!',V_SQLERRM,'结算->SP_JS_TSMZJD_YB2YY',pi_FWWDBH,pi_czyuan);
       	      COMMIT;
END SP_JS_TSMZJD_YB2YY_JM;
/
