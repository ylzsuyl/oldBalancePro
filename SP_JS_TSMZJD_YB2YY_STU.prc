CREATE OR REPLACE PROCEDURE SP_JS_TSMZJD_YB2YY_STU
(
  ...
  ERR_INS_MZJD        EXCEPTION;  --插入结算特殊门诊季度表时出错-7005
...
BEGIN
  ...
  SELECT COUNT(*) INTO LN_COUNT FROM VW_JS_TSMZJS_JM
   ...
   LOOP
      ...
      loop
         ...
         exit when c_2%notfound;
            ...
            BEGIN
            ln_count :=0;
            SELECT count(*),nvl(SUM(TSMZCS),0),nvl(SUM(fybfy0),0),nvl(SUM(fybzh0),0),
              ...
              into  ln_count,LN_TSMZCS,ln_fybfy0,ln_fybzh0,ln_zhbyzf,ln_jjbyzf,
                    ln_sbbyzf,LN_XJZF00, LN_ZHZF00,LN_JJZF00,LN_SBJJZF,LN_ZZF000
                FROM  Vw_Js_Tsmzjs_Stu
                WHERE TO_NUMBER(QSRQ00) >= TO_NUMBER(PI_QSRQ00) AND
                    TO_NUMBER(JZRQ00) <= TO_NUMBER(PI_JZRQ00) and
             ...
             if ln_count>0 then
               ...
               Begin
                ...
                INSERT INTO JS_TSMZJD_YB2YY_STU
                 (
                    NIAN00,QSRQ00,JZRQ00,FZXBH0,FWWDBH,WDJBBH,TSMZCS,
                 ...
                 VALUES
                   (
                    PI_NIAN00,PI_QSRQ00,PI_JZRQ00,ls_fzxbh0,pi_FWWDBH,pi_wdjbbh,nvl(LN_TSMZCS,0),
...
EXCEPTION   WHEN ERR_INS_MZJD THEN
               ...
               INSERT INTO JS_ERRLOG(CWH000,JDCWXX,XXCWXX,CCDF00,CZDX00,CZY000) VALUES
                (PO_SIGN,'插入结算特殊门诊月度表时出错!',V_SQLERRM,'结算->SP_JS_TSMZJD_YB2YY_STU',pi_FWWDBH,pi_czyuan);
               COMMIT;
            WHEN ERR_SEL_HBCS THEN
               PO_SIGN := -7412;
               ROLLBACK;
               INSERT INTO JS_ERRLOG(CWH000,JDCWXX,XXCWXX,CCDF00,CZDX00,CZY000) VALUES
                (PO_SIGN,'从表BM_JSBLB0_JM查找支付能力系数时错误!',V_SQLERRM,'结算->SP_JS_TSMZJD_YB2YY_STU',pi_FWWDBH,pi_czyuan);
               COMMIT;
            ...
            WHEN OTHERS THEN
               ROLLBACK;
               PO_SIGN := -7413;--门诊季度结算出错不详
               V_SQLERRM := SQLERRM;
               INSERT INTO JS_ERRLOG(CWH000,JDCWXX,XXCWXX,CCDF00,CZDX00,CZY000) VALUES
                (PO_SIGN,'特殊门诊结算发生以下错误!',V_SQLERRM,'结算->SP_JS_TSMZJD_YB2YY_STU',pi_FWWDBH,pi_czyuan);
               COMMIT;
END SP_JS_TSMZJD_YB2YY_STU;
/
